...
public:
  ...
  std::vector<std::pair<uint8_t, uint8_t>> calculate_possible_moves(const std::vector<std::vector<int8_t>>& board)
  {
    ...
    // Up-left
    if (y-2 >= BOARD_MIN && x-1 >= BOARD_MIN)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y-2, x-1});
    }
      
    // Up-right
    if (y-2 >= BOARD_MIN && x+1 <= BOARD_MAX)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y-2, x+1});
    }
    
    // Right-up
    if (y-1 >= BOARD_MIN && x+2 <= BOARD_MAX)
    {
    ...
    // Down-right
    if (y+2 <= BOARD_MAX && x+1 <= BOARD_MAX)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y+2, x+1});
    }
    
    // Down-left
    if (y+2 <= BOARD_MAX && x-1 >= BOARD_MIN)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y+2, x-1});
    }
    
    // Left-down
    if (y+1 <= BOARD_MAX && x-2 >= BOARD_MIN)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y+1, x-2});
    }
    
    // Left-up
    if (y-1 >= BOARD_MIN && x-2 >= BOARD_MIN)
    {
      moves.push_back(std::pair<uint8_t, uint8_t>{y-1, x-2});
    }
